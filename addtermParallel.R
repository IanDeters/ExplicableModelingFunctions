addtermParallel = function(object, scope, scale = 0, test = c('none', 'Chisq', 'F'), k = 2, sorted = FALSE, Cluster = NULL, trace = FALSE, ...)
{
  ...
  else
  {
    ...
    Results = foreach(i = seq(length(scope)), .combine = 'rbind') %dopar% {
      ...
      data.frame('tt' = tt, dfs = z$rank, dev = z$deviance)
      
    }
    
    dev[Results$tt] = Results$dev[Results$tt %in% names(dev)]
    dfs[Results$tt] = Results$dfs[Results$tt %in% names(dfs)]
